...
class DetailJadwalDaoImpl
{
    ...
    public function fetchFilterBeritaAcara(Jadwal $detailJadwal, $dosenFil, $semesterFil, $date1, $date2)
    {
        $link = PDOUtil::connectDb();
        if ($semesterFil != "" && $dosenFil != "") {
            if ($date1 && $date2 != "") {
                ...
                $stmt->bindValue(6, $date1);
                $stmt->bindValue(7, $date2);
            ...
            if ($date1 && $date2 != "") {
                $query = 'SELECT detail_jadwal.*  FROM detail_jadwal WHERE jadwal_kelas = ? AND jadwal_type = ? AND jadwal_MataKuliah_idMataKuliah = ? AND jadwal_Dosen_NIP = ? AND tanggal_pertemuan BETWEEN ? AND ?';
                $stmt = $link->prepare($query);
                $stmt->setFetchMode(PDO::FETCH_CLASS | PDO::FETCH_PROPS_LATE, 'DetailJadwal');
                $stmt->bindValue(1, $detailJadwal->getKelas());
                $stmt->bindValue(2, $detailJadwal->getType());
                $stmt->bindValue(3, $detailJadwal->getIdMatkul()->getIdMataKuliah());
                $stmt->bindValue(4, $dosenFil);
                $stmt->bindValue(5, $date1);
                $stmt->bindValue(6, $date2);
            ...
            if ($date1 && $date2 != "") {
                $query = 'SELECT detail_jadwal.*  FROM detail_jadwal WHERE jadwal_kelas = ? AND jadwal_type = ? AND jadwal_MataKuliah_idMataKuliah = ? AND jadwal_Semester_id_Semester = ? AND tanggal_pertemuan BETWEEN ? AND ?';
                $stmt = $link->prepare($query);
                $stmt->setFetchMode(PDO::FETCH_CLASS | PDO::FETCH_PROPS_LATE, 'DetailJadwal');
                $stmt->bindValue(1, $detailJadwal->getKelas());
                $stmt->bindValue(2, $detailJadwal->getType());
